...
class IMDBDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        return {
            ...
            "input_ids": encoding[
                "input_ids"
            ].flatten(),  # flatten() flattens a continguous range of dims in a tensor
...
#    test_data = imdb_dataset(train=False, test=True)
    ...
    raw_datasets = datasets.DatasetDict(
        {
            "train": datasets.DatasetBuilder.as_dataset(train_data),
            "test": datasets.DatasetBuilder.as_dataset(train_data),
        }
    )
...
def imdb_dataset(
    ...
    splits = [
        dir_
        for (requested, dir_) in [(train, train_directory), (test, test_directory)]
        if requested
    ]
