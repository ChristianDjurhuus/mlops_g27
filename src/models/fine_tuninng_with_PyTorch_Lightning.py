...
class ImdbDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.tokenized_datasets = self.datasets.map(self.convert_to_features, batched=True)
    ...
    def train_dataloader(self):
        return DataLoader(self.tokenized_datasets["train"].shuffle(seed=42).select(range(5)), self.batch_size)
...
class ImdbTransformer(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = get_scheduler("linear", optimizer, num_warmup_steps=0, num_training_steps=self.total_steps)
